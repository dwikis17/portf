...
export function BlogList() {
  ...
  const activeCategoryId = (categoriesQuery.data || []).some((item) => item.id === categoryParam)
    ? categoryParam
    : undefined;

  const blogsQuery = useQuery({
    queryKey: queryKeys.blogs.list({ page: 1, limit: 100, categoryId: activeCategoryId || null }),
    queryFn: () => queries.listBlogs(activeCategoryId)
  });
  ...
  return (
    <main className="min-h-[calc(100vh-108px)] animate-fade-in px-6 py-14 md:py-20">
      <section className="mx-auto grid w-full max-w-6xl gap-8">
        <header
          className="stagger-item flex flex-wrap items-end justify-between gap-4"
        ...
        <nav
          className="stagger-item flex flex-wrap gap-3"
        ...
        {blogsQuery.isLoading ? (
          <div
            className="stagger-item rounded-2xl border-4 border-zinc-950 bg-white p-6 text-sm font-semibold text-zinc-600 shadow-[0_4px_0_0_#09090b]"
          ...
          <div className="grid grid-cols-1 gap-6 md:grid-cols-2 xl:grid-cols-3">
            {blogs.map((blog, index) => (
              <div
                ...
                style={{ "--stagger-delay": `${210 + index * 55}ms` } as CSSProperties}
              >
                <ABlog blog={blog} />
              </div>
            ))}
